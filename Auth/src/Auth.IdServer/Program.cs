...
using Auth.IdServer;
using Auth.IdServer.Configurations;
using Auth.IdServer.Converters;
    ...
    using (var scope = app.Services.GetRequiredService<IServiceScopeFactory>().CreateScope())
    {
        using (var dbContext = scope.ServiceProvider.GetService<StoreDbContext>())
        {
            ...
            if (!dbContext.Realms.Any())
                dbContext.Realms.AddRange(Auth.IdServer.IdServerConfiguration.Realms);
            ...
            if (!dbContext.UmaPendingRequest.Any())
                dbContext.UmaPendingRequest.AddRange(Auth.IdServer.IdServerConfiguration.PendingRequests);

            if (!dbContext.UmaResources.Any())
                dbContext.UmaResources.AddRange(Auth.IdServer.IdServerConfiguration.Resources);

            if (!dbContext.GotifySessions.Any())
                dbContext.GotifySessions.AddRange(Auth.IdServer.IdServerConfiguration.Sessions);

            foreach (var language in Auth.IdServer.IdServerConfiguration.Languages)
                AddMissingLanguage(dbContext, language);

            foreach (var providerDefinition in Auth.IdServer.IdServerConfiguration.ProviderDefinitions)
            {
            ...
            if (!dbContext.IdentityProvisioningDefinitions.Any())
                dbContext.IdentityProvisioningDefinitions.AddRange(Auth.IdServer.IdServerConfiguration.IdentityProvisioningDefLst);

            if (!dbContext.IdentityProvisioningLst.Any())
                dbContext.IdentityProvisioningLst.AddRange(Auth.IdServer.IdServerConfiguration.GetIdentityProvisiongLst(scimBaseUrl));

            if (!dbContext.RegistrationWorkflows.Any())
                dbContext.RegistrationWorkflows.AddRange(Auth.IdServer.IdServerConfiguration.RegistrationWorkflows);
            ...
            if (!dbContext.CertificateAuthorities.Any())
                dbContext.CertificateAuthorities.AddRange(Auth.IdServer.IdServerConfiguration.CertificateAuthorities);

            if (!dbContext.PresentationDefinitions.Any())
                dbContext.PresentationDefinitions.AddRange(Auth.IdServer.IdServerConfiguration.PresentationDefinitions);

            if(!dbContext.FederationEntities.Any())
                dbContext.FederationEntities.AddRange(Auth.IdServer.IdServerConfiguration.FederationEntities);
        ...
        void AddMissingAuthenticationSchemeProviders(StoreDbContext dbContext)
        {
            ...
            {
                foreach (var provider in Auth.IdServer.IdServerConfiguration.Providers)
                {
        ...
        List<Scope> MigrateScopes(StoreDbContext dbContext, Realm masterRealm)
        {
            ...
            var unsupportedScopes = Auth.IdServer.IdServerConfiguration.Scopes.Where(s => !allScopeNames.Contains(s.Name));
        ...
        {
            var confClientIds = Auth.IdServer.IdServerConfiguration.Clients.Select(c => c.ClientId);
            var allClientIds = dbContext.Clients.Select(s => s.ClientId);
            var unknownClients = Auth.IdServer.IdServerConfiguration.Clients.Where(c => !allClientIds.Contains(c.ClientId));
            ...
            {
                var cl = Auth.IdServer.IdServerConfiguration.Clients.Single(c => c.ClientId == knownClient.ClientId);
